...
namespace MarkdownWikiGenerator
{
    ...
    public static class VSDocParser
    {
        ...
        private static string ResolveSeeElement(Match m, string ns)
        {
            ...
            if (!string.IsNullOrWhiteSpace(ns))
            {
                if (typeName.StartsWith(ns))
                {
                    ...
                    if (IsType(typeName))
                    {
                        return $"[{typeName}]({Regex.Replace(typeName, $"\\.(?:.(?!\\.))+$", me => me.Groups[0].Value.Replace(".", "/"))})";
                    }
                    else
                    {
                        return ResolveNonLinkElement(m, ns);
                    }
                }
            }
        ...
        {
            var match = Regex.Matches(typeName, @"(\.)");
        ...
        private static string ResolveNonLinkElement(Match m, string ns)
        {
            var typeName = m.Groups[1].Value;
            return Regex.Replace(typeName, $"\\.(?:.(?!\\.))+$", me => me.Groups[0].Value.Replace(".", "."));
        }
